...
const toolsDropdown = document.querySelector('.site-nav__dropdown');
const toolsToggle = document.querySelector('.site-nav__dropdown-toggle');
...
if (toolsDropdown && toolsToggle) {
    toolsToggle.addEventListener('click', (event) => {
        ...
        document.querySelectorAll('.site-nav__dropdown').forEach(dropdown => dropdown.classList.remove('open'));
        if (!isOpen) {
...
function closeNavDropdown() {
    ...
    navMenuBtn.setAttribute('aria-label', 'Open navigation menu');
...
function handleOutsideNavClick(event) {
    if (!navDropdown.contains(event.target) && !navMenuBtn.contains(event.target)) {
        closeNavDropdown();
    }
}
...
if (navMenuBtn && navDropdown) {
    navMenuBtn.addEventListener('click', function (e) {
        ...
        if (isOpen) {
            ...
            setTimeout(() => document.addEventListener('click', handleOutsideNavClick), 0);
        }
        e.stopPropagation();
    });
